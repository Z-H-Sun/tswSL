...
SEND_MESSAGE.call($hWndComboBox, CB_LIMITTEXT, 9, 0) # limit to 9 bytes
...
begin
  ...
  Object.send(:remove_const, :SAVEDAT_PATH) # undefine SAVEDAT_PATH
...
def slTEMP(save, num, filename=nil)
  filename = TIME_STAMP+(num+1).to_s if filename.nil?
  # Temporarily change the path to the data file
  WRITE_PROCESS.call($hPrc, BASE_ADDRESS+PATH_ADDRESS[8*save+num], filename, FILENAME_LEN, '    ')
  ...
  if defined?(SAVEDAT_PATH)
    ...
    for i in Dir.entries('.')
      ...
      if File.directory?(i) # consider 1st sub-folder
        for j in Dir.entries(i)
          next if j == '.' or j == '..'
          k = i+"\\"+j
          next if k.length > 9
          next if File.directory?(k)
          SEND_MESSAGE.call($hWndComboBox, CB_ADDSTRING, 0, k)
        end
      else
...
while GET_MESSAGE.call(msg, 0, 0, 0) != 0
  ...
  if offset > 0 and keyDown == 27 or keyDown == 13 # enter/esc
    if keyDown == 13
      fName = ' '*256
      len = SEND_MESSAGE.call($hWndComboBox, WM_GETTEXT, 255, fName)
      if len.zero?
        ...
        MESSAGE_BOX.call($hWnd, 'The filename length exceeds limit (9 bytes, note that each CJK char is 2-byte). It will be truncated to 9 bytes.', 'tswSL', MB_ICONEXCLAMATION)
      end
      SEND_MESSAGE.call($hWndText, WM_SETTEXT, 0, '') # clear status bar
      SEND_MESSAGE.call($hWndComboBox, CB_ADDSTRING, 0, fName[0, 9]) if SEND_MESSAGE.call($hWndComboBox, CB_FINDSTRINGEXACT, -1, fName[0, 9]) < 0 # non-existing
      slTEMP(@save, 1, "\\"+fName[0, 9])
    else
      SEND_MESSAGE.call($hWndText, WM_SETTEXT, 0, '') # clear status bar
    end
  ...
  if @save == 2 # exit
    ...
    if num == 8 # custom
      ...
      SEND_MESSAGE.call($hWndText, WM_SETTEXT, 0, "tswSL: Please input the filename to #{@save.zero? ? 'load' : 'save'} (9 bytes at max), and press ENTER:")
